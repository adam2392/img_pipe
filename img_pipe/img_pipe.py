...
class freeCoG:
    ...
    def __init__(self, subj, hem, zero_indexed_electrodes=True, fs_dir=os.environ['FREESURFER_HOME'],
    ...
    def get_recon(self, flag_T3='-3T', openmp_flag='-openmp 12', gpu_flag=''):
        ...
        '''
        os.system(
            'recon-all -subjid %s -sd %s -all %s %s %s' % (self.subj, self.subj_dir, flag_T3, openmp_flag, gpu_flag))
    ...
    def reg_img(self, source='CT.nii', target='orig.mgz', smooth=0., reg_type='rigid', interp='pv', xtol=0.0001,
                ftol=0.0001):
        ...
        ct_to_mri_reg = nipy.algorithms.registration.histogram_registration.HistogramRegistration(ctimg, mriimg,
                                                                                                  similarity='nmi',
                                                                                                  smooth=smooth,
                                                                                                  interp=interp)
    ...
    def interp_grid(self, nrows=16, ncols=16, grid_basename='hd_grid'):
        ...
        for i in np.arange(3):
            elecmatrix[corner_nums[0]:corner_nums[1] + 1, i] = np.linspace(elecmatrix[corner_nums[0], i],
                                                                           elecmatrix[corner_nums[1], i], nrows)
            elecmatrix[corner_nums[2]:corner_nums[3] + 1, i] = np.linspace(elecmatrix[corner_nums[2], i],
                                                                           elecmatrix[corner_nums[3], i], nrows)
        ...
        for row in np.arange(nrows):
            for i in np.arange(3):
                elecmatrix[grid[row, :], i] = np.linspace(elecmatrix[row, i],
                                                          elecmatrix[row + (grid[0, -1] - grid[0, 0]), i], ncols)
    ...
    def project_electrodes(self, elecfile_prefix='hd_grid', use_mean_normal=True, \
        ...
        if elecfile_prefix == 'OFC_grid':
            self.make_roi_mesh('OFC',
                               ['lateralorbitofrontal', 'medialorbitofrontal', 'rostralmiddlefrontal', 'parsorbitalis',
        ...
        cortex = scipy.io.loadmat(
            os.path.join(self.subj_dir, self.subj, 'Meshes', '%s_%s_trivert.mat' % (self.hem, surf_type)))
        ...
        elecmatrix = scipy.io.loadmat(
            os.path.join(self.subj_dir, self.subj, 'elecs', 'individual_elecs', '%s_orig.mat' % (elecfile_prefix)))[
            'elecmatrix']
        print(direction)
    ...
    def get_clinical_grid(self, elecfile_prefix='hd_grid'):
        ...
        scipy.io.savemat(os.path.join(self.elecs_dir, 'individual_elecs', 'clinical_' + elecfile_prefix + '.mat'),
    ...
    def warp_all(self, elecfile_prefix='TDT_elecs_all', warp_depths=True, warp_surface=True,
        ...
        if 'depth' in orig_elecs['anatomy'][:, 2] and warp_depths:
            ...
            if not os.path.isfile(os.path.join(self.subj_dir, self.subj, 'cvs',
                                               'combined_to' + template + '_elreg_afteraseg-norm.tm3d')):
                self.get_cvsWarp(template)
            else:
                print('%s registration file already created, proceeding to apply the depth warp' % (
                    os.path.join(self.subj_dir, self.subj, 'cvs',
                                 'combined_to' + template + '_elreg_afteraseg-norm.tm3d')))
        ...
        if warp_surface:
            ...
            if not os.path.isfile(
                ...
                print('Found %s, not running surface warp again' % (
                    os.path.join(self.subj_dir, self.subj, 'elecs', elecfile_prefix + '_surface_warped.mat')))
        ...
        if warp_depths and warp_surface:
            scipy.io.savemat(elecfile_warped,
                             {'elecmatrix': orig_elecs['elecmatrix'], 'anatomy': orig_elecs['anatomy']})
    ...
    def compute_surface_warp(self, elecmatrix, anatomy=None, labelpath='tmp/', basename='',
        ...
        if anatomy is not None:
            surface_indices = np.array(
                list(set(np.where(anatomy[:, 2] != 'depth')[0]) & set(
                    np.where(np.all(~np.isnan(elecmatrix), axis=1))[0])),
                dtype='int64')
        else:
    ...
    def check_depth_warps(self, elecfile_prefix='TDT_elecs_all', template='cvs_avg35_inMNI152',
        ...
        cvs_img = nib.freesurfer.load(
            os.path.join(self.subj_dir, template, 'mri', 'aparc' + depth_atlas_nm + '+aseg.mgz'))
    ...
    def apply_xfm(self, xfm_dir='mri/transforms', xfm_file='talairach.xfm',
        ...
        if file_type == 'elecs':
            ...
            warnings.warn("Please specify file_type = 'surf' or file_type = 'elecs'.")
    ...
    def plot_elec(self, orig_coords, warped_coords, subj_dat, cvs_dat, elec_num, pdf):
        ...
        cvs_vox_CRS = np.array([warped_coords[0] - offset, warped_coords[1] - offset, warped_coords[2] - offset],
                               dtype='int')
        ...
        subj_vox_CRS = np.array([orig_coords[0] - offset, orig_coords[1] - offset, orig_coords[2] - offset],
                                dtype='int')
        ...
        if cvs_dat[cvs_vox_CRS[0], cvs_vox_CRS[1], cvs_vox_CRS[2]] != subj_dat[
            subj_vox_CRS[0], subj_vox_CRS[1], subj_vox_CRS[2]]:
            result = 'MISMATCH'
            color = 'r'
        else:
            result = 'Match'
    ...
    def get_elecs(self, elecfile_prefix='TDT_elecs_all', roi=None):
        ...
        if os.path.isfile(elecfile):
            ...
            e = {'elecmatrix': elecmatrix,
                 'anatomy': anatomy}  # {'anatomy': anatomy, 'elecmatrix': elecmatrix, 'eleclabels': eleclabels}
        else:
    ...
    def plot_brain(self, rois=[
        roi(name='pial', color=(0.8, 0.8, 0.8), opacity=1.0, representation='surface', gaussian=False)], elecs=None,
    ...
    def plot_recon_anatomy(self, elecfile_prefix='TDT_elecs_all', template=None, showfig=True, screenshot=False,
        ...
        if self.hem == 'lh' or self.hem == 'rh':
            ...
            mesh, mlab = ctmr_brain_plot.ctmr_gauss_plot(a['lh']['tri'], a['lh']['vert'], color=(0.8, 0.8, 0.8),
                                                         opacity=opacity)
            mesh, mlab = ctmr_brain_plot.ctmr_gauss_plot(a['rh']['tri'], a['rh']['vert'], color=(0.8, 0.8, 0.8),
        ...
        for b in brain_areas:
            ...
            if b[0][0] != 'NaN':
                ...
                if b[0][0:3] != 'ctx' and b[0][0:4] != 'Left' and b[0][0:5] != 'Right' and b[0][0:5] != 'Brain' and b[
                    0] != 'Unknown':
                ...
                if this_label != '' and this_label != 'NaN':
                    if this_label not in cmap:
                        ...
                        el_color = matplotlib.cm.get_cmap('viridis').colors[
                            int(float(np.where(brain_areas == b)[0]) / float(len(brain_areas)))]
                    else:
    ...
    def plot_erps(self, erp_matrix, elecfile_prefix='TDT_elecs_all', time_scale_factor=0.03,
        ...
        for c in range(num_channels):
            if anat_colored:
                ...
                if b != 'NaN':
                    this_label = b[0]
                    if b[0][0:3] != 'ctx' and b[0][0:4] != 'Left' and b[0][0:5] != 'Right' and b[0][0:5] != 'Brain' and \
                            b[0] != 'Unknown':
                    ...
                    if this_label != '':
                        if this_label not in cmap:
                            # in case the label was manually assigned, and not found in the LUT colormap dictionary
                            erp_color = matplotlib.cm.get_cmap('viridis').colors[
                                int(float(np.where(brain_areas == b)[0]) / float(len(brain_areas)))]
                        else:
            ...
            if self.hem == 'lh':
                label_offset = -2.0
                y_array = ((np.array(
                    [i for i in range(num_timepoints / 2 - num_timepoints, num_timepoints / 2)])) * time_scale_factor +
                           elec_coord[1])[::-1]
            elif self.hem == 'rh':
                label_offset = 2.0
                y_array = ((np.array(
    ...
    def plot_recon_anatomy_compare_warped(self, template='cvs_avg35_inMNI152', elecfile_prefix='TDT_elecs_all',
        ...
        template_e['elecmatrix'][:, 1] = template_e['elecmatrix'][:, 1] + (
        ...
        template_mesh, mlab = ctmr_brain_plot.ctmr_gauss_plot(template_brain['tri'], template_brain['vert'],
                                                              color=(0.8, 0.8, 0.8), new_fig=False)
    ...
    def make_roi_mesh(self, roi_name, label_list, hem=None, showfig=False, save=True):
        ...
        if hem == None:
            if self.hem != 'lh' and self.hem != 'rh':
                print(
                    'You need to specify which hemisphere this ROI is in. Please try again and specify the hemisphere in the hem argument.')
                return
            else:
                hem = self.hem
        ...
        tri_row_inds = np.sort(np.array(list(set(np.where(np.in1d(cortex['tri'][:, 0], vertnums))[0]) & set(
            np.where(np.in1d(cortex['tri'][:, 1], vertnums))[0]) & set(
            np.where(np.in1d(cortex['tri'][:, 2], vertnums))[0]))))
    ...
    def plot_all_surface_rois(self, bgcolor=(0, 0, 0), size=(1200, 900), color_dict=None, screenshot=False, showfig=True, **kwargs):
        """
...
def _method_args_to_parser(method, parser):
    ...
    for arg_number in range(len(method_argnames)):
        if arg_number < required_args:
            ...
            parser.add_argument('--' + method_argnames[arg_number], type=type(args_values[arg_number]),
